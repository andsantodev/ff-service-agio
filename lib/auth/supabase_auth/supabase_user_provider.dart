...
class ServiceAgioSupabaseUser extends BaseAuthUser {
  ServiceAgioSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> serviceAgioSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ServiceAgioSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
